    ...
    while time.time() - start_time < 120:  # 2-minute timeout
        ...
        if "context" in inquiries_response and "sheriff_challenge" in inquiries_response["context"]:
            ...
            if challenge_type in ["sms", "email"] and challenge_status == "issued":
                ...
                if isinstance(challenge_response, dict) and challenge_response.get("status") == "validated":
                    break
                else:
                    print(f"Challenge response did not validate: {challenge_response}")
    ...
    if os.path.isfile(pickle_path):
        ...
        if store_session:
            try:
                ...
                print("ERROR: There was an issue loading pickle file. Authentication may be expired - logging in normally.", file=get_output())
